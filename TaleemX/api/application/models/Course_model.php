...
class Course_model extends CI_Model
{
    ...
    public function lessonquizbysection($sectionid, $student_id, $course_id) 
    {
        ...
        $this->db->join('online_course_quiz', 'online_course_quiz.id=course_lesson_quiz_order.lesson_quiz_id AND course_lesson_quiz_order.type="quiz"', 'left');
        $this->db->join('online_course_lesson', 'online_course_lesson.id=course_lesson_quiz_order.lesson_quiz_id AND course_lesson_quiz_order.type="lesson"', 'left');     
        $this->db->join('online_course_assignment','online_course_assignment.id=course_lesson_quiz_order.lesson_quiz_id AND course_lesson_quiz_order.type="assignment"','left');
        $this->db->join('online_course_exam','online_course_exam.id=course_lesson_quiz_order.lesson_quiz_id AND course_lesson_quiz_order.type="exam"','left');
        ...
        foreach ($result as $key => $result_value) {
            $result[$key]->video_url = ($result_value->type == 'lesson') ? (string) $result_value->video_url : '';
            $result[$key]->video_provider = ($result_value->type == 'lesson') ? (string) $result_value->video_provider : '';
            $result[$key]->video_id = ($result_value->type == 'lesson') ? (string) $result_value->video_id : '';
            $result[$key]->lesson_type = ($result_value->type == 'lesson') ? (string) $result_value->lesson_type : '';
            $result[$key]->duration = ($result_value->type == 'lesson') ? (string) $result_value->duration : '';
