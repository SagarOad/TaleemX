...
class Webservice extends CI_Controller
{
    ...
    private function decodeEntitiesDeep($text)
    {
        $text = (string) $text;
        if ($text === '') {
            return '';
        ...
        $previous = null;
        $iteration = 0;
        while ($previous !== $text && $iteration < 5) {
            $previous = $text;
    ...
    private function sanitizeInvisibleChars($text)
    {
        $text = (string) $text;
        if ($text === '') {
            return '';
        ...
        $text = str_replace(
            array("\xC2\xA0", "\xE2\x80\x8B", "\xE2\x80\x8C", "\xE2\x80\x8D", "\xEF\xBB\xBF"),
            array(' ', '', '', '', ''),
            $text
        );
        $text = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/u', '', $text);
        $text = preg_replace('/[\x{25A1}\x{2610}\x{2611}\x{2612}\x{2B1C}\x{2B1B}]/u', '', $text);
        return $text;
    }

    private function cleanExamText($text)
    {
        $text = $this->decodeEntitiesDeep($text);
        $text = preg_replace('/<br\s*\/?>/i', ' ', $text);
        $text = strip_tags($text);
        $text = $this->sanitizeInvisibleChars($text);
        $text = preg_replace('/[ \t]+/u', ' ', $text);
        $text = preg_replace('/\s*\n\s*/u', "\n", $text);
        return trim((string) $text);
    }

    private function cleanExamHtml($text)
    {
        $text = $this->decodeEntitiesDeep($text);
        $text = preg_replace('/\s(data-[a-z0-9\-]+|class|style)="[^"]*"/i', '', $text);
        $text = $this->sanitizeInvisibleChars($text);
        return trim((string) $text);
    }

    private function extractMediaFromHtml($text)
    {
        $text = (string) $text;
        if ($text === '') {
            return array();
        }
        $decoded = $this->decodeEntitiesDeep($text);
        $images = array();
        if (preg_match_all('/<img[^>]+src=[\'"]([^\'"]+)[\'"]/i', $decoded, $matches)) {
    ...
    private function extractAltTextFromHtml($text)
    {
        $text = (string) $text;
        if ($text === '') {
            return '';
        ...
        $decoded = $this->decodeEntitiesDeep($text);
        $alts = array();
        if (preg_match_all('/(?:alt|title|aria-label)=[\'"]([^\'"]+)[\'"]/i', $decoded, $matches)) {
    ...
    private function cleanExamTextWithFallback($text, $placeholder = '')
    {
        $plain = $this->cleanExamText($text);
        if ($plain !== '') {
            return $plain;
        }
        $alt = $this->cleanExamText($this->extractAltTextFromHtml($text));
        if ($alt !== '') {
            return $alt;
        }
        $images = $this->extractMediaFromHtml($text);
        if (!empty($images)) {
            return '[image]';
        }
        return $placeholder;
    ...
    private function normalizeExamQuestion($q_val)
    {
        if (!is_object($q_val)) {
            return;
        }

        $qtype = (string)($q_val->question_type ?? '');

        $question_raw   = $q_val->question ?? '';
        $question_plain = $this->cleanExamTextWithFallback($question_raw);
        $question_html  = $this->cleanExamHtml($question_raw);
        $question_media = $this->extractMediaFromHtml($question_raw);

        if ($question_plain === '') {
            $question_plain = ($qtype === 'descriptive') ? 'Write your answer below.' : 'Question content not available.';
        }

        $opt_a_plain = $this->cleanExamTextWithFallback($q_val->opt_a ?? '');
        $opt_b_plain = $this->cleanExamTextWithFallback($q_val->opt_b ?? '');
        $opt_c_plain = $this->cleanExamTextWithFallback($q_val->opt_c ?? '');
        $opt_d_plain = $this->cleanExamTextWithFallback($q_val->opt_d ?? '');
        $opt_e_plain = $this->cleanExamTextWithFallback($q_val->opt_e ?? '');

        if ($qtype === 'true_false') {
            if ($opt_a_plain === '') { $opt_a_plain = 'True'; }
            if ($opt_b_plain === '') { $opt_b_plain = 'False'; }
        }

        $q_val->question      = $question_plain;
        $q_val->question_html = $question_html;
        $q_val->question_plain = $question_plain;
        $q_val->question_media = $question_media;

        $q_val->opt_a = $opt_a_plain;
        $q_val->opt_b = $opt_b_plain;
        $q_val->opt_c = $opt_c_plain;
        $q_val->opt_d = $opt_d_plain;
        $q_val->opt_e = $opt_e_plain;

        $q_val->opt_a_plain = $opt_a_plain;
        $q_val->opt_b_plain = $opt_b_plain;
        $q_val->opt_c_plain = $opt_c_plain;
        $q_val->opt_d_plain = $opt_d_plain;
        $q_val->opt_e_plain = $opt_e_plain;

        $q_val->opt_a_media = $this->extractMediaFromHtml($q_val->opt_a ?? '');
        $q_val->opt_b_media = $this->extractMediaFromHtml($q_val->opt_b ?? '');
        $q_val->opt_c_media = $this->extractMediaFromHtml($q_val->opt_c ?? '');
        $q_val->opt_d_media = $this->extractMediaFromHtml($q_val->opt_d ?? '');
        $q_val->opt_e_media = $this->extractMediaFromHtml($q_val->opt_e ?? '');

        $q_val->allow_multiple = ($qtype === 'multichoice') ? 1 : 0;
        $q_val->input_control  = ($qtype === 'multichoice') ? 'checkbox' : (($qtype === 'descriptive') ? 'textarea' : 'radio');

        $options = array();
        if ($opt_a_plain !== '') { $options[] = array('key' => 'opt_a', 'value' => $opt_a_plain); }
        if ($opt_b_plain !== '') { $options[] = array('key' => 'opt_b', 'value' => $opt_b_plain); }
        if ($opt_c_plain !== '') { $options[] = array('key' => 'opt_c', 'value' => $opt_c_plain); }
        if ($opt_d_plain !== '') { $options[] = array('key' => 'opt_d', 'value' => $opt_d_plain); }
        if ($opt_e_plain !== '') { $options[] = array('key' => 'opt_e', 'value' => $opt_e_plain); }

        if (empty($options) && $qtype !== 'descriptive') {
            $options[] = array('key' => 'opt_a', 'value' => 'Option A');
            $options[] = array('key' => 'opt_b', 'value' => 'Option B');
        }

        $q_val->options = $options;
    }
    ...
    public function test()
    {
        ...
        if ($method !== 'GET') {
            json_output(405, array('status' => 405, 'message' => 'Method not allowed.'));
        }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if (!empty($exam['questions'])) {
                        foreach ($exam['questions'] as $q_key => $q_val) {
                            $this->normalizeExamQuestion($exam['questions'][$q_key]);
                        }
                    }
                ...
                if ($response['status'] == 200) {
                    ...
                    if (!empty($resp['question_result'])) {
                        ...
                        foreach ($resp['question_result'] as $result_key => $question_value) {
                            ...
                            $this->normalizeExamQuestion($resp['question_result'][$result_key]);
	...
	public function saveOnlineExam()
	{
		...
		if (is_string($question_rows)) {
			$decoded_rows = json_decode($question_rows, true);
			if (is_array($decoded_rows)) {
				$question_rows = $decoded_rows;
			}
		}

		if (!is_array($question_rows) && !empty($params['rows'])) {
			$question_rows = is_string($params['rows']) ? json_decode($params['rows'], true) : $params['rows'];
		}

		if (!is_array($question_rows)) {
			foreach (['answers', 'question_rows', 'data', 'questions'] as $alt_key) {
				$candidate = $this->input->post($alt_key);
				if (is_string($candidate)) {
					$candidate = json_decode($candidate, true);
				}
				if (!is_array($candidate) && !empty($params[$alt_key])) {
					$candidate = is_string($params[$alt_key]) ? json_decode($params[$alt_key], true) : $params[$alt_key];
				}
				if (is_array($candidate) && !empty($candidate)) {
					$question_rows = $candidate;
					break;
				}
			}
		}
		...
		if (!is_array($question_rows) || empty($question_rows)) {
			json_output(400, ['status' => 0, 'msg' => 'rows payload is required']);
			return;
		}

		$question_rows = array_values(array_filter($question_rows, 'is_array'));
		if (empty($question_rows)) {
			json_output(400, ['status' => 0, 'msg' => 'rows payload is required']);
			return;
		}

		$saas_enabled = (bool) $this->config->item('saas_enabled');
		$file_keys = [];
		...
		if ($saas_enabled && !empty($file_keys)) {
			$limit_status = $this->saasvalidation->getResourceLimit('storage');

			if (is_array($limit_status) && !empty($limit_status['status']) &&
		...
		if (empty($onlineexam_student_id) && !empty($params['onlineexam_student_id'])) {
			$onlineexam_student_id = $params['onlineexam_student_id'];
		}
		if (empty($onlineexam_student_id) && !empty($question_rows[0]['onlineexam_student_id'])) {
			$onlineexam_student_id = $question_rows[0]['onlineexam_student_id'];
		}
		if (empty($onlineexam_student_id)) {
			$onlineexam_student_id = $this->input->get_request_header('Onlineexam-Student-ID', true);
		}
		if (empty($onlineexam_student_id)) {
			json_output(400, ['status' => 0, 'msg' => 'onlineexam_student_id is required']);
			return;
		}
    ...
    public function addStaffRating()
    {
        ...
        $params = array_merge($json_params, $post_data);

        $user_id  = $params['user_id'] ?? $params['userid'] ?? $params['userId'] ?? null;
        $staff_id = $params['staff_id'] ?? $params['staffid'] ?? $params['staffId'] ?? $params['employee_id'] ?? null;
        $rate     = $params['rate'] ?? $params['rating'] ?? $params['stars'] ?? null;
        $comment  = $params['comment'] ?? $params['review'] ?? $params['feedback'] ?? '';
        ...
        if (!empty($staff_id)) {
            if (is_numeric($staff_id)) {
                $resolved_staff_id = (int) $staff_id;
            } else {
                $staff_row = $this->db->select('id')
                    ->from('staff')
                    ->where('employee_id', (string) $staff_id)
                    ->limit(1)
                    ->get()
                    ->row();
                if (!empty($staff_row)) {
                    $resolved_staff_id = (int) $staff_row->id;
                }
            }
        }
        ...
        if (empty($user_id) || !is_numeric($user_id)) {
            $errors['user_id'] = 'user_id is required and must be numeric';
        }
        if (empty($resolved_staff_id)) {
            $errors['staff_id'] = 'staff_id is required (numeric id or employee_id)';
        }
        if (!is_numeric($rate) || (int) $rate < 1 || (int) $rate > 5) {
            $errors['rate'] = 'rate is required and must be between 1 and 5';
        }

        if (!empty($errors)) {
            json_output(200, array('status' => 0, 'msg' => 'Validation failed', 'error' => $errors));
            return;
        }
        ...
        try {
            if (!empty($existing)) {
                $this->db->where('id', $existing->id)->update('staff_rating', array(
                    'rate'    => $data['rate'],
                    'comment' => $data['comment'],
                    'role'    => $data['role'],
                    'status'  => $data['status'],
                ));
                json_output(200, array('status' => 1, 'msg' => 'updated', 'id' => (int) $existing->id));
                return;
            }

            $this->db->insert('staff_rating', $data);
            $new_id = $this->db->insert_id();
            json_output(200, array('status' => 1, 'msg' => 'inserted', 'id' => (int) $new_id));
        } catch (Exception $e) {
            json_output(200, array('status' => 0, 'msg' => 'Failed to save rating', 'error' => array('db' => $e->getMessage())));
        }
    }
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    $course_id  = $payload['course_id'] ?? $this->input->post('course_id');
                    $student_id = $payload['student_id'] ?? $this->input->post('student_id');
                    if (empty($student_id)) {
                        $student_id = $this->input->get_request_header('Student-ID', true);
                    }

                    if (empty($course_id) || empty($student_id)) {
                        json_output(400, array('status' => 0, 'message' => 'course_id and student_id are required.'));
                        return;
                    }
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    $modulearray = array('class_timetable', 'syllabus_status', 'attendance', 'examinations', 'student_timeline', 'mydocuments', 'behaviour_records');
    ...
    {
        json_output(410, array('status' => 410, 'message' => 'CBSE exam module has been removed.'));
        return;
    }
    ...
    public function saveCourseAssignment()
    {
        ...
        $data = array_merge($json_params, $post_data);
        ...
        $submission_id = $data['id']
            ?? $data['submission_id']
            ?? $data['assignment_submit_id']
            ?? 0;

        $errors = array();
        if (empty(trim((string)$assignment_id))) {
            $errors['assignmentid'] = 'assignment id is required';
        }
        if (empty(trim((string)$student_id))) {
        ...
        $file_field = null;
        foreach (array('file', 'docs', 'attachment', 'document', 'upload') as $candidate) {
            if (isset($_FILES[$candidate]) && !empty($_FILES[$candidate]['name'])) {
                $file_field = $candidate;
                break;
            }
        }

        if ($file_field !== null && $file_field !== 'file') {
        ...
        if ($file_field !== null) {
            ...
            if (!empty($file_info['error']) && $file_info['error'] !== UPLOAD_ERR_OK) {
                json_output(200, array('status' => '0', 'msg' => 'File upload error', 'error' => array('file' => 'Upload error code: ' . $file_info['error'])));
                return;
            }

            $file_check = $this->validateAssignmentFile($file_info);
            if ($file_check !== true) {
                json_output(200, array('status' => '0', 'msg' => $file_check, 'error' => array('file' => $file_check)));
                return;
            }
            ...
            $img_name = $this->media_storage->fileupload('file', $upload_path);
            if (IsNullOrEmptyString($img_name)) {
                json_output(200, array('status' => '0', 'msg' => 'File could not be saved', 'error' => array('file' => 'File could not be saved')));
                return;
            }
        } else if (!empty($data['docs']) && is_string($data['docs'])) {
            $img_name = $data['docs'];
        }

        $data_insert = array(
            'id'            => ($submission_id > 0) ? (int) $submission_id : 0,
        ...
        $saved_id = $this->course_model->save_assignment($data_insert);
    ...
    private function validateAssignmentFile($file_info)
    {
        if (empty($file_info) || empty($file_info['name']) || empty($file_info['tmp_name'])) {
            return 'File attachment missing';
        }
        ...
        $allowed_extensions = array_filter(array_map('trim', array_map('strtolower', explode(',', (string) $filetype->file_extension))));
        if (!empty($allowed_extensions) && !in_array($ext, $allowed_extensions, true)) {
            return 'Extension not allowed: ' . $ext;
        }

        if (!empty($filetype->file_size) && !empty($file_info['size']) && $file_info['size'] > (int) $filetype->file_size) {
            $limit_mb = number_format($filetype->file_size / 1048576, 2);
            return $this->lang->line('file_size_shoud_be_less_than') . $limit_mb . ' MB';
        }
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if (!empty($resp['question_result'])) {
                        foreach ($resp['question_result'] as $q_key => $q_val) {
                            $this->normalizeExamQuestion($resp['question_result'][$q_key]);
                        }
                    }
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
		            ...
		            $question_rows = $params['rows'] ?? $this->input->post('rows');
                    if (!is_array($question_rows) || empty($question_rows)) {
                        json_output(400, array('status' => 0, 'msg' => 'rows payload is required'));
                        return;
                    }

                    $student_id = $params['student_id'] ?? $this->input->post('student_id');
                    if (empty($student_id)) {
                        $student_id = $this->input->get_request_header('Student-ID', true);
                    }
                    $guest_id = $params['guest_id'] ?? $this->input->post('guest_id') ?? 0;
                    $usertype = $params['usertype'] ?? ($params['user_type'] ?? ($this->input->post('usertype') ?? $this->input->post('user_type')));
                    $exam_id = $params['exam_id'] ?? $this->input->post('exam_id');

                    if (empty($exam_id) || empty($usertype)) {
                        json_output(400, array('status' => 0, 'msg' => 'exam_id and usertype are required'));
                        return;
                    }
                    if ($usertype === 'student' && empty($student_id)) {
                        json_output(400, array('status' => 0, 'msg' => 'student_id is required for student usertype'));
                        return;
                    }
                    if ($usertype === 'guest' && empty($guest_id)) {
                        json_output(400, array('status' => 0, 'msg' => 'guest_id is required for guest usertype'));
                        return;
                    }
