...
class Customlib
{
    ...
    public function __construct()
    {
        ...
        $this->CI->load->helper(array('custom', 'saudi_phone'));
    ...
    public function getRTL()
    {
        ...
        if ($user && isset($user['is_rtl']) && $user['is_rtl'] !== 'disabled') {
            // Use the active language's short code when available (e.g. ar, he, fa, ur)
            // instead of hard-coding Arabic. Falls back to 'ar' for backward compatibility.
            $lang_code = 'ar';
            if (!empty($user['language']['lang_short_code'])) {
                $lang_code = $user['language']['lang_short_code'];
            }
            $rtl = "dir='rtl' lang='" . htmlspecialchars($lang_code, ENT_QUOTES) . "'";
        }
