...
class Media_storage
{
    ...
    {
        $dbg = function ($msg) {
            log_message('error', '[UPLOAD_DEBUG] ' . $msg);
        };

        if (!isset($_FILES[$media_name])) {
            $dbg("no \$_FILES[$media_name] set");
            return null;
        }

        $file_error = $_FILES[$media_name]['error'];
        if ($file_error !== UPLOAD_ERR_OK) {
            $dbg("PHP upload error code=$file_error for $media_name");
            return null;
        }
        ...
        $folder_path_raw = $this->_CI->customlib->getFolderPath();
        $upload_dir = $folder_path_raw . $upload_path;

        $dbg("cwd=" . getcwd()
            . " | folder_path_from_session=" . var_export($folder_path_raw, true)
            . " | upload_path_arg=" . var_export($upload_path, true)
            . " | computed_upload_dir=" . var_export($upload_dir, true)
            . " | tmp_name=" . var_export($tmp_name, true)
            . " | original_name=" . var_export($original_name, true)
            . " | tmp_exists=" . (file_exists($tmp_name) ? 'yes' : 'no'));

        if (!is_dir($upload_dir)) {
            $mk = @mkdir($upload_dir, 0755, true);
            $dbg("upload_dir did not exist; mkdir result=" . ($mk ? 'ok' : 'FAILED') . " realpath=" . var_export(realpath($upload_dir), true));
            if (!$mk) {
                return null;
            }
        }
        ...
        if (!is_writable($upload_dir)) {
            $dbg("upload_dir NOT writable: $upload_dir");
            return null;
        }
        ...
        if (!is_uploaded_file($tmp_name)) {
            $dbg("is_uploaded_file() returned FALSE for $tmp_name");
            return null;
        }

        if (!move_uploaded_file($tmp_name, $destination)) {
            $err = error_get_last();
            $dbg("move_uploaded_file FAILED. last_error=" . var_export($err, true));
            return null;
        }

        $dbg("SUCCESS. file saved. file_exists=" . (file_exists($destination) ? 'yes' : 'no') . " size=" . (file_exists($destination) ? filesize($destination) : -1));
        return $file_name;
    }
