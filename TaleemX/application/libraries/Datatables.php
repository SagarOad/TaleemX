...
class Datatables
{
    ...
    public array $colomn_search_Array = [];
    ...
    private function get_filtering()
    {
        ...
        if ($sSearch != '' && $this->searchable != '') {
            ...
            foreach ($column as $key => $col) {
                $col         = strtolower($col);
                $col         = strstr($col, ' as ', true) ?: $col;
                $this->colomn_search_Array[] = $col;
            }

            if ($sSearch != "") {
                for ($i = 0; $i < count($this->colomn_search_Array); $i++) {
                    if ($i == 0) {
                        $this->ci->db->group_start();
                        $this->ci->db->like($this->colomn_search_Array[$i], $sSearch);
                    } else {
                        $this->ci->db->or_like($this->colomn_search_Array[$i], $sSearch);
                    }

                    if (count($this->colomn_search_Array) - 1 == $i) //last loop
                    {
                        $this->ci->db->group_end();
                    }
                    //close bracket
