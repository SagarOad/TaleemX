    ...
    public function send($tokens, $msg, $action = "")
    {
        try {
            if (!file_exists($this->key_file_path)) {
                log_message('error', 'Pushnotification key file not found: ' . $this->key_file_path);
                return false;
            }

            $credentials_json = json_decode(file_get_contents($this->key_file_path), true);
            if (empty($credentials_json)) {
                log_message('error', 'Pushnotification key file is empty or invalid JSON.');
                return false;
            }

            $creadentials = new ServiceAccountCredentials('https://www.googleapis.com/auth/firebase.messaging', $credentials_json);
            ...
            // Keep business flow running (e.g., homework create) even if push auth fails.
