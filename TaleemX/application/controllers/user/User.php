...
class User extends Student_Controller
{
    ...
    {
        // Pull short_code too so the session + <html lang="xx"> tag stay in sync.
        $language_name = $this->db
            ->select('languages.language, languages.short_code, languages.is_rtl')
            ->from('languages')
            ->where('id', $lang_id)
            ->get()
            ->row_array();
        ...
        $student['language'] = array(
            'lang_id'         => $lang_id,
            'lang_short_code' => isset($language_name['short_code']) ? $language_name['short_code'] : 'en',
            'language'        => isset($language_name['language'])   ? $language_name['language']   : '',
        );

        $student['is_rtl'] = ((int) ($language_name['is_rtl'] ?? 0) === 1) ? 'enabled' : 'disabled';
