...
class Studentcourse extends Student_Controller
{
    ...
    /**
     * Same-origin proxy for the Caption AI microservice.
     *
     ...
     *
     * Loads the stored transcript text for the given lesson
     * (video_transcripts.full_transcript) and forwards it to the external
     * /caption-ai endpoint, mirroring the admin/askai proxy pattern.
     *
     * Response JSON:
     *   { "answer": "..." }  on success
     *   { "error":  "..." }  on failure (with non-2xx HTTP code)
     */
    public function caption_ai()
    {
        ...
        if ($action !== 'summarize' && $action !== 'explain') {
            ...
            echo json_encode(array('error' => 'Invalid action. Use "summarize" or "explain".'));
            return;
        }
        if ($lesson_id <= 0) {
            http_response_code(400);
            echo json_encode(array('error' => 'Missing or invalid lesson_id.'));
            return;
        }

        $this->load->model('video_transcript_model');
        $text = $this->video_transcript_model->get_full_text('lesson', $lesson_id);
        if ($text === '') {
            http_response_code(409);
            echo json_encode(array(
                'error' => 'No transcript is saved for this lesson yet. Please ask an admin to extract subtitles for this video first.',
            ));
            return;
        }
