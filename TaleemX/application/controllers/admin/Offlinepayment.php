...
class Offlinepayment extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('offlinePayment_model', 'setting_model', 'category_model', 'studentfeemaster_model'));
    }
    ...
    public function getpayment()
    {
        ...
        // Guard: a missing / empty recordid would cause the model to return
        // result() (an array of all rows) instead of a single row, and then
        // break every $payment->... access below.
        if (empty($recordid)) {
            echo json_encode(array(
                'status' => '0',
                'error'  => $this->lang->line('error_occurred_please_try_again'),
                'page'   => '',
            ));
            return;
        }
        ...
        // Guard: id didn't match any record.
        ...
        if (IsNullOrEmptyString($payment->student_transport_fee_id)) {
            ...
            // Only hit the fee-master lookup if we actually have the two ids it
            // needs. Otherwise the hand-built SQL would become
            // `WHERE student_fees_master.id= and fee_groups_feetype.id=` and
            // throw a 1064.
            $fee_detail = null;
            if (!empty($payment->student_fees_master_id) && !empty($payment->fee_groups_feetype_id)) {
                $fee_detail = $this->studentfeemaster_model->getFeesByStudentFeeMasterAndFeetype(
                    $payment->student_fees_master_id,
                    $payment->fee_groups_feetype_id
                );
            }

            $total_fine_amount_paid = 0;

            if (!empty($fee_detail) && isset($fee_detail->amount_detail) && isJSON($fee_detail->amount_detail)) {
            ...
            if (!empty($fee_detail) && isset($fee_detail->due_date) && ($fee_detail->due_date != "0000-00-00" && $fee_detail->due_date != null) && (strtotime($fee_detail->due_date) < strtotime($payment->payment_date))) {
