...
class Addons extends Admin_Controller
{
    ...
    function installNew($addon_folder, $product_id, $addon_details)
    {
        ...
        if ($copy_folder) { //if folder copied successfully then run install sql
            ...
            if ($sql_success['status']) {
                $this->activateInstalledAddonMenus($addon_details);
                $install_sql = true;
    ...
    function updateVersion($addon_versions)
    {
        ...
        foreach ($addon_versions->nxt_versions as $nxt_version_key => $nxt_version_value) {
            ...
            if ($copy_folder) { //if folder copied successfully then run install sql
                ...
                if ($sql_success['status']) {
                    $this->activateInstalledAddonMenus($addon_versions);
                    $install_sql = true;
                } else {  //rollback installed sql 
    ...
    private function activateInstalledAddonMenus($addon_details)
    {
        if (empty($addon_details->short_name)) {
            return;
        }

        $addon_short_name = $addon_details->short_name;

        // Some addon SQL ships menu rows as inactive. If a row is meant for the
        // sidebar, make it visible immediately after the addon installs.
        $this->db->where('product_name', $addon_short_name);
        $this->db->where('sidebar_display', 1);
        $this->db->update('sidebar_menus', array('is_active' => 1));

        $this->db->where('addon_permission', $addon_short_name);
        $this->db->update('sidebar_sub_menus', array('is_active' => 1));

        $this->db->select('sidebar_menu_id, access_permissions');
        $this->db->from('sidebar_sub_menus');
        $this->db->where('addon_permission', $addon_short_name);
        $submenu_query = $this->db->get();

        foreach ($submenu_query->result() as $submenu) {
            if (empty($submenu->sidebar_menu_id) || empty($submenu->access_permissions)) {
                continue;
            }

            $this->db->select('id, access_permissions');
            $this->db->from('sidebar_menus');
            $this->db->where('id', $submenu->sidebar_menu_id);
            $menu = $this->db->get()->row();

            if (!$menu || strpos($menu->access_permissions, $submenu->access_permissions) !== false) {
                continue;
            }

            $access_permissions = trim($menu->access_permissions);
            $access_permissions .= ($access_permissions === '' ? '' : ' || ') . trim($submenu->access_permissions);

            $this->db->where('id', $menu->id);
            $this->db->update('sidebar_menus', array('is_active' => 1, 'access_permissions' => $access_permissions));
        }
    }
