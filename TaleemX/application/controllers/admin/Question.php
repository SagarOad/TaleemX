...
class Question extends Admin_Controller
{
    ...
    public function getDatatable()
    {
        ...
        if (!is_object($staffrole)) {
            $staffrole = (object) array('id' => 0);
        }
        $admin_session            = $this->session->userdata('admin');
        $superadmin_restriction   = is_array($admin_session) && isset($admin_session['superadmin_restriction'])
            ? $admin_session['superadmin_restriction']
            : '';
        $class_id              = $this->input->post('class_id');
        $section_id            = $this->input->post('section_id');
        ...
        if (!is_object($question_dt)) {
            $question_dt = (object) array(
        ...
        if (!empty($question_dt->data)) {
            foreach ($question_dt->data as $key => $value) {
                ...
                $row[] = (isset($value->class_name) ? $value->class_name : '')." (".(isset($value->section_name) ? $value->section_name : '').")";
                $row[] = (isset($value->name) ? $value->name : '') .' '. $code ;
                $q_type_key = isset($value->question_type) ? $value->question_type : '';
                $row[] = ($q_type_key !== "" && isset($question_type[$q_type_key])) ? $question_type[$q_type_key] : "";
                $q_level_key = isset($value->level) ? $value->level : '';
                $row[] = ($q_level_key !== "" && isset($question_level[$q_level_key])) ? $question_level[$q_level_key] : "";
                $row[] = readmorelink(isset($value->question) ? $value->question : '', site_url('admin/question/read/' . (isset($value->id) ? $value->id : '')));                
                
                $staff_name    = isset($value->staff_name) ? $value->staff_name : '';
                $staff_surname = isset($value->staff_surname) ? $value->staff_surname : '';
                $employee_id   = isset($value->employee_id) ? $value->employee_id : '';
                $created_role  = isset($value->created_role) ? $value->created_role : 0;
                $staff_label   = $staff_name . ' ' . $staff_surname . ' (' . $employee_id . ')';

                if($superadmin_restriction == 'disabled'){
                    if(isset($staffrole->id) && $staffrole->id == 7){
                        $row[] = $staff_label;
                    }else{
                        if($created_role != 7){
                            $row[] = $staff_label;
        ...
        $json_data = array(
            "draw"            => intval(isset($question_dt->draw) ? $question_dt->draw : $this->input->post('draw')),
            "recordsTotal"    => intval(isset($question_dt->recordsTotal) ? $question_dt->recordsTotal : 0),
            "recordsFiltered" => intval(isset($question_dt->recordsFiltered) ? $question_dt->recordsFiltered : 0),
            "data"            => $dt_data,
        );
