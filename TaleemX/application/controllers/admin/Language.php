...
class Language extends Admin_Controller
{
    ...
    public function user_language($lang_id)
    {
        ...
        // Rebuild the admin session so language + RTL flag actually persist.
        // Using array access on $this->session->userdata (a magic __get) does
        // NOT write back to $_SESSION in CI3, so we must call set_userdata().
        $session['language'] = [
            'lang_id'         => $setting['lang_id'],
            'lang_short_code' => $setting['short_code'],
            'language'        => $setting['language'],
        ];
        $session['is_rtl'] = ((int) $setting['is_rtl'] === 1) ? 'enabled' : 'disabled';
