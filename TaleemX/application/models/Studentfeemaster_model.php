...
class Studentfeemaster_model extends MY_Model
{
    ...
    {
        // Bail out early if either id is missing — the callers already expect
        // a falsy return in that case, and concatenating an empty value into
        // the raw SQL below produces a 1064 syntax error.
        if (empty($student_fees_master_id) || empty($fee_groups_feetype_id)) {
            return null;
        }

        $student_fees_master_id  = (int) $student_fees_master_id;
...
public function getFeeSessionGroupId($student_fees_master_id)
{
	...
	public function getFeeByInvoice($invoice_id, $sub_invoice_id)
    {
        ...
        return $this->getOfflineBankFeeByInvoice($invoice_id, $sub_invoice_id);
    }

    public function getOfflineBankFeeByInvoice($invoice_id, $sub_invoice_id)
    {
        $invoice_id     = (int) $invoice_id;
        $sub_invoice_id = (int) $sub_invoice_id;

        if ($invoice_id <= 0 || $sub_invoice_id <= 0) {
            return false;
        }
        ...
        $payment_id = $invoice_id . "/" . $sub_invoice_id;

        $this->db->select($invoice_id . ' as id', false);
        $this->db->select('student_fees_deposite.amount_detail,students.id as std_id,students.firstname,students.middlename,students.lastname,students.admission_no,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,student_session.session_id,offline_fees_payments.student_session_id,"Offline Bank Payment" as name,"Bank Payment" as type,offline_fees_payments.reference as code,0 as is_system,0 as student_fees_master_amount,offline_fees_payments.amount as offline_amount,offline_fees_payments.payment_date as offline_payment_date,offline_fees_payments.approved_by', false);
        $this->db->from('offline_fees_payments');
        $this->db->join('student_fees_deposite', 'student_fees_deposite.id = ' . $invoice_id, 'left');
        $this->db->join('student_session', 'student_session.id = offline_fees_payments.student_session_id');
        $this->db->join('classes', 'classes.id = student_session.class_id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('students', 'students.id = student_session.student_id');
        $this->db->where('offline_fees_payments.invoice_id', $payment_id);
        $this->db->where('offline_fees_payments.is_active', 1);

        if (!empty($class_section_array)) {
            $this->db->group_start();
            foreach ($class_section_array as $class_id => $section_ids) {
        ...
        $result         = $q->row();
        $amount_details = json_decode($result->amount_detail);
        $amount_array   = is_object($amount_details) ? (array) $amount_details : array();

        if (!array_key_exists($sub_invoice_id, $amount_array)) {
            $result->amount_detail = json_encode(array(
                $sub_invoice_id => array(
                    'amount'          => $result->offline_amount,
                    'amount_discount' => 0,
                    'amount_fine'     => 0,
                    'date'            => $result->offline_payment_date,
                    'description'     => 'Amount credited through offline bank payment',
                    'payment_mode'    => 'bank_payment',
                    'received_by'     => $result->approved_by,
                    'inv_no'          => $sub_invoice_id,
                ),
            ));
        }
