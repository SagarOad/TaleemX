...
class Notification_model extends MY_Model
{
    ...
    {
        // Permanently purge any license-registration notices from the notifications table.
        $this->db->query("DELETE FROM send_notification WHERE LOWER(COALESCE(title, '')) LIKE '%unregistered%' OR LOWER(COALESCE(title, '')) LIKE '%purchase code%' OR LOWER(COALESCE(title, '')) LIKE '%register your purchase%' OR LOWER(COALESCE(message, '')) LIKE '%unregistered%' OR LOWER(COALESCE(message, '')) LIKE '%purchase code%' OR LOWER(COALESCE(message, '')) LIKE '%register your purchase%' OR LOWER(COALESCE(message, '')) LIKE '%purchasemodal%'");

        $sql = "select send_notification.* from send_notification INNER JOIN notification_roles on notification_roles.send_notification_id = send_notification.id left JOIN read_notification on read_notification.staff_id=" . $this->db->escape($staffid) . " and read_notification.notification_id = send_notification.id WHERE send_notification.created_id !=" . $this->db->escape($staffid) . " and send_notification.visible_staff='yes' and read_notification.id IS NULL and notification_roles.role_id=" . $this->db->escape($role_id) . "
            AND LOWER(send_notification.title) NOT LIKE '%unregistered%'
            AND LOWER(send_notification.title) NOT LIKE '%purchase code%'
            AND LOWER(send_notification.title) NOT LIKE '%register your purchase%'
            AND LOWER(COALESCE(send_notification.message, '')) NOT LIKE '%unregistered%'
            AND LOWER(COALESCE(send_notification.message, '')) NOT LIKE '%purchase code%'
            AND LOWER(COALESCE(send_notification.message, '')) NOT LIKE '%register your purchase%'
            order by send_notification.id desc";

        $query = $this->db->query($sql);
        $notifications = $query->result();

        // Defense-in-depth: also filter any remaining license-related notices in PHP.
        ...
        foreach ($notifications as $notice) {
            $raw_title   = isset($notice->title) ? (string) $notice->title : '';
            $raw_message = isset($notice->message) ? (string) $notice->message : '';

            $title   = strtolower(strip_tags(html_entity_decode($raw_title, ENT_QUOTES | ENT_HTML5, 'UTF-8')));
            $message = strtolower(strip_tags(html_entity_decode($raw_message, ENT_QUOTES | ENT_HTML5, 'UTF-8')));

            $is_blocked = false;
            foreach ($blocked_patterns as $pattern) {
                if (strpos($title, $pattern) !== false || strpos($message, $pattern) !== false
                    || stripos($raw_title, $pattern) !== false || stripos($raw_message, $pattern) !== false) {
                    $is_blocked = true;
                    break;
                }
            }
