...
<script src="<?php echo base_url(); ?>backend/dist/js/sidebar-active-scroll.js"></script>
<!--nprogress-->
...
<script type="text/javascript">
    // Ensure every Bootstrap modal has a visible close button.
    (function ($) {
        function ensureModalCloseButton($modal) {
            if (!$modal || !$modal.length) return;
            var $content = $modal.find('.modal-content').first();
            if (!$content.length) return;
            if ($modal.find('.ss-modal-auto-close').length) return;
            if (window.getComputedStyle($content[0]).position === 'static') {
                $content.css('position', 'relative');
            }
            $content.append('<button type="button" class="ss-modal-auto-close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>');
        }
        $(document).on('shown.bs.modal', '.modal', function () {
            ensureModalCloseButton($(this));
        });
        $(function () {
            $('.modal').each(function () { ensureModalCloseButton($(this)); });
        });
    })(jQuery);
...
<?php
// =============================================================================
// Floating "Ask AI" button — appears on every admin page (hidden on the
// Ask AI screen itself). Clicking it opens admin/askai.
// Uses shared --ss-* tokens so it follows brand color, light/dark, and RTL.
// =============================================================================
$askai_current_class  = strtolower((string) $this->router->fetch_class());
$askai_on_askai_page  = ($askai_current_class === 'askai');
if (!$askai_on_askai_page):
?>
<style type="text/css">
    .askai-fab {
        position: fixed;
        ...
        box-shadow: 0 10px 28px rgba(var(--ss-color-brown-rgb, 68, 47, 36), 0.50),
    ...
    .askai-fab:focus {
        ...
        box-shadow: 0 14px 34px rgba(var(--ss-color-brown-rgb, 68, 47, 36), 0.60),
                    0 6px 14px rgba(0, 0, 0, 0.22);
        filter: brightness(1.05);
        outline: none;
    }
    .askai-fab:active { transform: translateY(0); }
    ...
    /* Soft "pulse" ring to draw attention */
    .askai-fab__icon::before {
        content: "";
        position: absolute;
        inset: -4px;
        border-radius: 50%;
        ...
        opacity: 0;
        animation: askaiFabPulse 2.4s ease-out infinite;
    }
    @keyframes askaiFabPulse {
    ...
    .askai-fab__tip {
        ...
        inset-inline-end: 0;
        padding: 6px 10px;
        background: var(--ss-color-neutral-900, #212121);
        color: #fff;
        font-size: 12px;
        border-radius: 6px;
        white-space: nowrap;
    ...
    .askai-fab__tip::after {
        content: "";
        position: absolute;
        top: 100%;
        inset-inline-end: 18px;
        border: 5px solid transparent;
        border-top-color: var(--ss-color-neutral-900, #212121);
    }
    .askai-fab:hover .askai-fab__tip,
    .askai-fab:focus .askai-fab__tip {
        opacity: 1;
        transform: translateY(0);
    }

    /* Compact version on small screens — icon-only circle */
    @media (max-width: 576px) {
        ...
        .askai-fab__label { display: none; }
        .askai-fab__icon { width: 22px; height: 22px; background: transparent; font-size: 18px; }
    }

    /* Hide during print */
    @media print {
        .askai-fab { display: none !important; }
    }
</style>

<a href="<?php echo base_url('admin/askai'); ?>" class="askai-fab" id="askaiFab"
   data-placement="top" aria-label="Ask AI">
    <span class="askai-fab__icon" aria-hidden="true"><i class="fa fa-magic"></i></span>
    <span class="askai-fab__label">Ask AI</span>
    <span class="askai-fab__tip">Need help? Ask the AI assistant</span>
</a>
<?php endif; ?>
