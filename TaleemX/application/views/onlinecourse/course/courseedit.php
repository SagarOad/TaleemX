...
if (!empty($allTeacherList)) {
    ...
    ?>
            ...
            <?php
                ...
                $__current_provider  = isset($coursesList["course_provider"]) ? $coursesList["course_provider"] : '';
                // Preserve legacy provider values (vimeo/html5/s3_bucket) on existing courses
                // by keeping them as a selectable option if they are no longer in the simplified list.
                if ($__current_provider !== '' && !isset($__preview_providers[$__current_provider]) && isset($course_provider[$__current_provider])) {
                    $__preview_providers[$__current_provider] = $course_provider[$__current_provider];
                }
                $__is_file_provider = ($__current_provider === 'file' || $__current_provider === 's3_bucket');
                $__url_div_class    = $__is_file_provider ? 'form-group hide' : 'form-group';
                $__file_div_class   = $__is_file_provider ? 'col-md-8' : 'col-md-8 hide';
            ?>
            <div class="row">
                <div class="col-md-12">
                ...
                <div class="col-md-4"> 
                 <div class="form-group">
                    <select  id="course_provider_edit" onchange="checkCourseProviderEdit()" onclick="checkCourseProviderEdit()" name="course_provider" class="form-control">
                    <?php
                        if (!empty($__preview_providers)) {
                            foreach ($__preview_providers as $key => $course_provider_value) { ?>
                                <option value="<?php echo $key ?>" <?php if ($key == $__current_provider) {echo "selected";}?>> <?php echo $course_provider_value; ?></option>
                            <?php }
                        } ?>
                   </select>
                  </div>
                ...
                <div class="col-md-8">
                    <div class="<?php echo $__url_div_class; ?>" id="course_url_div_edit">
                 ...
                 <div class="<?php echo $__file_div_class; ?>" id="s3_file_div_edit">
                  <div class="form-group">
                    <input autofocus="" id="s3_file" name="s3_file"  placeholder="" type="file" class="form-control filestyle" />
                    <?php if ($__is_file_provider && !empty($coursesList['video_id'])) { ?>
                        <p class="help-block" style="margin-top:8px;">
                            <i class="fa fa-file-video-o"></i>
                            <strong>Current file:</strong>
                            <?php if ($__current_provider === 'file') { ?>
                                <a href="<?php echo base_url(); ?>uploads/course/course_preview/<?php echo rawurlencode($coursesList['video_id']); ?>" target="_blank" rel="noopener">
                                    <?php echo htmlspecialchars($coursesList['video_id'], ENT_QUOTES, 'UTF-8'); ?>
                                </a>
                            <?php } else { ?>
                                <span><?php echo htmlspecialchars($coursesList['video_id'], ENT_QUOTES, 'UTF-8'); ?></span>
                            <?php } ?>
                            <br><small class="text-muted">Leave empty to keep the current file, or choose a new file to replace it.</small>
                        </p>
                    <?php } ?>
                  </div>
              </div>                
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <input type="hidden" id="edit_course_id_hidden" value="<?php echo (int) $courseID; ?>">
                        <?php
                            // Pre-compute the public URL for a locally uploaded preview file
                            // so the Extract button can use it even though `course_url` is empty
                            // for the `file` provider.
                            $__file_preview_url = '';
                            if (($__current_provider === 'file') && !empty($coursesList['video_id'])) {
                                $__file_preview_url = base_url() . 'uploads/course/course_preview/'
                                    . rawurlencode($coursesList['video_id']);
                            }
                        ?>
                        <button type="button"
                                id="edit_course_extract_subtitles_btn"
                                class="btn btn-default"
                                data-course-id="<?php echo (int) $courseID; ?>"
                                data-file-url="<?php echo htmlspecialchars($__file_preview_url, ENT_QUOTES, 'UTF-8'); ?>">
                            <i class="fa fa-closed-captioning"></i> Extract subtitles
                        </button>
                        <span id="edit_course_extract_subtitles_status"
                              class="hide"
...
<script>
    ...
    (function () {
        var editCourseId = parseInt($('#edit_course_id_hidden').val(), 10) || 0;
        if (editCourseId > 0 && typeof window.refreshCourseSubtitleStatus === 'function') {
            window.refreshCourseSubtitleStatus(editCourseId);
        }
    })();
</script>
